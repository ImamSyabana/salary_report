...
#def read_excel(file_path):
    df_full = pd.read_excel(file_path, header=None, sheet_name="KRW")
    ...
    new_column_names = [
        'No', 'Nama', 'No_rek', 'Jabatan', 'n_hadir', 'UM_per_hadir', 'Insentif(unit)', 'Insentif(tarif)', 
        'Gaji_pokok', 'UM_total', 'Tunj_jab', 'Tunj_hdr', 'instv_DLR', 'Tunj_komunikasi', 
    ...
    def get_end_row_driver(df):
        ...
        if nan_mask.any():
            # Get the index label of the first row that is NaN
            first_nan_label = df_driver_data[nan_mask].index[0]
            
            # Get the positional index (integer) of that label
            stop_position = df_driver_data.index.get_loc(first_nan_label)
